...
output:
  knitr:::html_vignette:
    toc: yes
---
```{r loadlibrary, include=FALSE, cache=TRUE}
...
```{r readtif, warnings=FALSE, collapse=TRUE, cache=TRUE}
...
`readClassTIF()` is a special version of `readTIF()` for images with a very low number of intensity values, e.g. masks or segmentes images.
...
```{r rwadMBP, warnings=FALSE, collapse=TRUE, cache=TRUE}

...
img<-function(x,z=NULL,ch=NULL,mask=NULL,col="grey",low=NULL,up=NULL,...)
{
  ...
  x[x<0]<-0
  x[x>1]<-1
  lattice::levelplot(array(0,dim(x)),axes=FALSE,xlab="",ylab="",col="black",...)
  #graphics::image(1:dim(x)[1],1:dim(x)[2],array(0,dim(x)),axes=FALSE,xlab="",ylab="",col="black",...)
  lattice::levelplot(x,col.regions=colo,zlim=c(0,1))
  #graphics::image(1:dim(x)[1],1:dim(x)[2],x,col=colo,zlim=c(0,1),add=TRUE)
}
...
img.rgb<-function(x,z=NULL,mask=NULL, n=100,low,up,...)
{
  ...
  lattice::levelplot(1:dim(x)[2],1:dim(x)[1],xx,axes=FALSE,col=colo,xlab="",ylab="",zlim=c(1,n*n*n),...)
}
